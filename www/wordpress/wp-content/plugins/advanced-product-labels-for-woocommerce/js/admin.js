...
(function ($){
    ...
    $(document).ready( function () {
        ...
        $(document).on('change', '[name="br_labels[template]"]', function() {
            if( ( typeof $('[name="br_labels[template]"]:checked').val() !== 'undefined' ) &&
                $('[name="br_labels[template]"]:checked').val().indexOf("image") >= 0 ) {
                $('.advanced-labels-font-family').closest('tr').hide();
            } else {
                $('.advanced-labels-font-family').closest('tr').show();
            }
            $('#size_multiplier').val(1);
            $('[name="br_labels[image]"]').closest('div').find('.berocket_remove_image').click();
            $('.br_range').change();
        }).change();
        ...
        $(document).on('input', '#size_multiplier, #size_multiplier_num_num, [name="br_labels[text]"]', changePreview );
    ...
    function br_hide_selected_template_options() {
        ...
        if( $selected_template.length ) {
            ...
            if ( template_hide.includes('image_width') ) {
                ...
                $('#size_multiplier').closest('tr').hide();
            }
